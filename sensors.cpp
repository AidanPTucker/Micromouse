...
// Initialize global variables (will be overwritten at startup)
...
void setupGyro() {
  // Running in IMUPLUS to ignore the magnetic field generated by the motors
...
void calibrateSideSensors() {
  ...
  // Take 20 samples to get a perfectly stable average
  for(int i = 0; i < 20; i++) {
  ...
  // Calculate dynamic thresholds based on the solid wall
  int avg_wall = (TARGET_SIDE_L + TARGET_SIDE_R) / 2;
  
  // A wall is present if reading is > 30% of a perfect wall
  IR_WALL_THRESHOLD = avg_wall * 0.30; 
  
  // A gap is present if reading falls below 15% of a perfect wall
...
bool isGyroCalibrated() {
  ...
  // Status is 0 (uncalibrated) to 3 (fully calibrated)
  return (gyro >= 3); 
}
...
void realignGyro(float true_grid_angle) {
  ...
  // Slowly pull our offset so that (raw_yaw - offset) equals the true_grid_angle
  // We use a slow filter (0.05) so a single bad IR reading doesn't violently snap the compass
  ...
  gyro_offset -= (error * 0.05); // Gently nudge the offset to fix the drift
